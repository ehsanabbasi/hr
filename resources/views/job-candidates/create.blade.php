<x-dashboard-layout>
    <x-slot name="title">Add Candidate for {{ $job->title }}</x-slot>
    
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
            <h1 class="text-2xl font-bold mb-6">Add Candidate for {{ $job->title }}</h1>
            
            <form method="POST" action="{{ route('jobs.candidates.store', $job) }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-4">
                    <label for="resume" class="block mb-2 text-sm font-medium text-gray-700">Resume (PDF)</label>
                    <input id="resume" type="file" name="resume" accept=".pdf"
                           class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <p class="text-sm text-gray-500 mt-1">Upload PDF file only (max 10MB)</p>
                    @error('resume')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-end">
                    <a href="{{ route('jobs.candidates.index', $job) }}" class="text-gray-600 mr-4">Cancel</a>
