<x-dashboard-layout>
    <x-slot name="title">Edit Certificate</x-slot>
            ...
            <form method="POST" action="{{ route('certificates.update', $certificate) }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $certificate->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input id="due_date" type="date" name="due_date" value="{{ old('due_date', $certificate->due_date ? $certificate->due_date->format('Y-m-d') : '') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @error('due_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_mandatory" value="1" {{ old('is_mandatory', $certificate->is_mandatory) ? 'checked' : '' }}
                ...
                <div class="flex items-center justify-end">
                    ...
                    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Update Certificate</button>
                </div>
            </form>
        </div>
    </div>
